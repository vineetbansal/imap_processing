...
_LSK_PATH = str(IMAP_SPICE_DATA_PATH / "lsk" / "naif0012.tls")


@contextmanager
def _load_lsk() -> Generator[None, None, None]:
    try:
        spiceypy.kinfo(_LSK_PATH)
        already_loaded = True
    except spiceypy.utils.exceptions.NotFoundError:
        already_loaded = False
        spiceypy.furnsh(_LSK_PATH)
    try:
        yield
    finally:
        if not already_loaded:
            spiceypy.unload(_LSK_PATH)


# The UTC time of the J2000 epoch (TT=0). Note that using this constant for
# naive TTJ2000ns → UTC conversion (by simple addition) results in a ~5-second
# error because TT is uniform but UTC has had ~5 leap seconds since J2000.
# TODO: Implement a function for converting CDF epoch to UTC correctly.
#     see github ticket #1208
with _load_lsk():
    TTJ2000_EPOCH = np.datetime64(
        spiceypy.et2utc(spiceypy.unitim(0, "TT", "ET"), "ISOC", 9), "ns"
    )
...
def ttj2000ns_to_datetime64(
    tt_ns: npt.ArrayLike,
) -> np.datetime64 | npt.NDArray[np.datetime64]:
    """
    Convert TT J2000 nanoseconds to numpy datetime64 (UTC).

    Uses SPICE for the conversion so that leap seconds are correctly accounted
    for. This is the correct alternative to naive arithmetic with TTJ2000_EPOCH,
    which would accumulate a ~5-second error due to leap seconds.
    ...
    numpy.datetime64 or numpy.ndarray[numpy.datetime64]
        UTC time(s) as numpy.datetime64.
    """
    et = ttj2000ns_to_et(tt_ns)
    return np.array(et_to_utc(et, "ISOC"), dtype=np.datetime64)[()]
...
def epoch_to_doy(epoch: np.ndarray) -> npt.NDArray:
    ...
    """
    datetimes = ttj2000ns_to_datetime64(epoch.data)
    return np.array(
        [datetime.fromisoformat(str(dt)).timetuple().tm_yday for dt in datetimes]
    )
