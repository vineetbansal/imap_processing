...
def l1b_bgrates_and_goodtimes(  # noqa: PLR0912
    ...
    for pivot_angles in c.PIVOT_ANGLES.values():
        if pivot_angles.min <= pivot <= pivot_angles.max:
            if pivot_angles.bg_rate_ram is not None:
                bg_rate_ram_nominal = pivot_angles.bg_rate_ram
            if pivot_angles.bg_rate_anti_ram is not None:
                bg_rate_anti_ram_nominal = pivot_angles.bg_rate_anti_ram
            break
